...
function vNextHandleOnOpen_() {
  try {
    ...
    // legacy menuへの危険なfallbackを遮断する。
    var detectedMode = typeof vNextDetectBookMode_ === 'function' ? String(vNextDetectBookMode_() || '').toUpperCase() : '';
    if (detectedMode === 'TEMPLATE') {
      if (typeof vNextBuildTemplateMenu_ === 'function') return Boolean(vNextBuildTemplateMenu_());
      return true;
    }
    if (detectedMode === 'LEGACY') return false;
    // A simple onOpen trigger can have a blank ActiveUser email. The client
    // menu itself is a recovery path and must be added before identity checks.
    if (detectedMode === 'CLIENT') return vNextBuildClientMenu_();
    var context = vNextUxGetBookContext_();
    if (!context || !context.mode) return false;
    if (String(context.mode).toUpperCase() === 'ADMIN') {
      if (typeof vNextBuildAdminMenu_ === 'function') return Boolean(vNextBuildAdminMenu_());
      return false;
    }
    if (String(context.mode).toUpperCase() !== 'CLIENT') return false;
    return vNextBuildClientMenu_();
  } catch (err) {
    Logger.log('vNextHandleOnOpen_ error: ' + vNextUxErrorText_(err));
    return false;
  }
}
...
function vNextRequestForecast() {
  try {
    ...
    if (typeof vNextEngineRunForecast === 'function') {
