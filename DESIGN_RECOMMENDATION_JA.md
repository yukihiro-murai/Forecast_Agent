# 売上予測スクリプト 設計書 v2.3.9（2.3.46 同期 / budget-nav-guard）
...
対象実装：`Forecast_Agent.js`（VERSION='2.3.46-dev' / BUILD_STAGE='budget-nav-guard'）
設計書版：v2.3.9（v2.3.8 からのドキュメント改訂）。
DLM_BUILD_STAGE は 'v8-step3c3c-1'（DLMロジック無変更のため据え置き）。

本改訂は、設計書 v2.3.8 が記述する `2.3.41-dev / report-text-compress` から現行コード
`2.3.46-dev / budget-nav-guard` までの差分を同期する。コードはこの間に5増分（2.3.42〜2.3.46）進んでいるが、
いずれも **メニュー再編・予算策定欄の表示追加・ナビゲーション/GUIDE 表示の整合** のみで、スコア算出式・予測コア・
履歴スキーマは無変更、A-9 の年度合計・月次 P10/P50/P90 は不変である。

**未反映（前提）**：別途生成済みの `config-blank-guard`（VERSION 予定 2.3.47 / CONFIG数値読取の空セルガード + 死にコード
削除）は本書執筆時点で**未適用・未検証**のため、本設計書には反映していない（設計書は確認済みコードにのみ同期する方針）。
そのため §3 の getCfg 空セルの罠、および §12 の coerceMatch 死にコードは、2.3.46 時点の現存挙動として記載する。

### 0.0 v2.3.8 → v2.3.9 の更新点（この改訂で反映したもの）
...
- `gotoBudgetEntry`（A-10 予算を策定）は OUTPUT の予算策定欄（H24）へカーソルを移動するナビゲーションのみ。
- OUTPUT シートが無い場合は注意ダイアログで終了。OUTPUT はあるが **A-9 未実行（タイトル空）** の場合も、
  空の H24 へ飛ばさず「先に A-9 を実行」ダイアログで終了する（=本ガード追加）。
- A-9 実行後は OUTPUT を前面化し H24 をアクティブにしてトースト表示。
...
- **OUTPUT 混合セクションに予算策定欄（H/I/J）を追加**。**A-10「予算を策定」はその欄（H24）へのナビゲーションのみ**で予測不変。
...
- **A-10 予算を策定（`gotoBudgetEntry`）**：OUTPUT の予算欄（H24）へ移動するナビゲーションのみ（予測非影響 / §9.6）。
...
- **予算策定欄（H/I 手入力）の A-9 再実行をまたいだ保持**（現状は `resetOutputSheet_` で都度ワイプ＝一回入力前提。§9.6・§12）。

### 1.4 メニュー構成（実装同期 / 2.3.46）
...
- **A-10 予算を策定（gotoBudgetEntry）** ← OUTPUT 予算欄 H24 へのナビゲーションのみ（予測非影響）
...
- **const 既定値は CONFIG セル欠落/不読時のフォールバック**。clasp push だけでは既存bookの CONFIG セルは更新されないため、
  const 既定の変更（overlay-cap-raise / spot-bg-sensitivity-down 等）を発効させるには A-1 再生成か該当セルの手動修正が必要。

### 3.2 主要キー（抜粋・現行同期）
- AI_WEIGHT / **AI_MAX_ABS_EFFECT（既定0.05＝±5%）** / AI_MISSING_CONFIDENCE_DEFAULT（既定0.5 / 0で不採用）
...
- **QUAL_SUBJECTIVE_MONTHLY_CAP（既定0.40）** / QUAL_CALIBRATION_ENABLED
...
### 3.3 既知の読取上の罠（現存 / config-blank-guard で是正予定・未適用）
`readModelTuningFromConfig_` 内の `getCfg(key, def)` と `readAiMissingConfidenceDefault_` は、CONFIG のセルが
**空文字列('')** のとき `Number('')===0 → isFinite(0)===true` により「0 という上書き値」を採用し、既定値ではなく
クランプ下限へ化ける（calibration-blank-override-fix と同型）。例：`QUAL_SUBJECTIVE_MONTHLY_CAP` 空 → 0.40 ではなく 0.01、
`AI_WEIGHT` 空 → 0、`AI_MISSING_CONFIDENCE_DEFAULT` 空 → 0.5 ではなく 0（=欠落 event 行は不採用）。
**正常にセットアップ済み（全セルシード）の book では発火しない**（空セルが無いため）。空/欠落を既定へフォールバックする
修正（`config-blank-guard`）が別プロンプトで用意済みだが、本書時点では未適用のため現存挙動として記載する（§12）。
...
- ※ report-text-compress（2.3.41）・dashboard-menu-to-b〜budget-nav-guard（2.3.42〜2.3.46）は予測値に影響しないため本KPIへの影響は無い。
...
- 現行コードは **VERSION='2.3.46-dev' / BUILD_STAGE='budget-nav-guard'**。DLM_BUILD_STAGE は 'v8-step3c3c-1'（据え置き）。
- 本改訂（設計書 v2.3.9）は 2.3.42〜2.3.46 の5増分（dashboard-menu-to-b / output-budget-columns / budget-entry-polish /
  guide-a10-sync / budget-nav-guard）を反映する。**いずれも表示・メニュー・ナビゲーションのみで予測値は不変**。
- 設計書ドリフトは既知の再発リスク。リリースごとに doc sync を独立タスクとして扱う。今後は stage 確定のたびに doc を同期する。
- **次期予定**：`config-blank-guard`（CONFIG数値読取の空セルガード + coerceMatch 死にコード削除）は別プロンプトで用意済み。
  適用・検証後に設計書 §3.3・§12 から「現存の罠/死にコード」記述を解消し、VERSION='2.3.47-dev' / BUILD_STAGE='config-blank-guard' へ同期する。
...
### 9.6 予算策定欄（output-budget-columns / budget-entry-polish / budget-nav-guard）【予測不変】
...
- **レイアウト（A-9 出力時）**
  ...
  - 各セクションのチャートは **L列（col 12）開始**（混合は L25）。K列（col 11）を余白に取り、H/I/J（8〜10列）と重ならない。
  - `buildOUTPUT_` は A-1 再セットアップ時に旧チャートを除去（A-9 後の再生成位置 L25 と食い違わない）。
- **A-10 予算を策定（gotoBudgetEntry）**：OUTPUT を前面化し H24 をアクティブにするナビゲーションのみ。
  OUTPUT 不在は注意ダイアログ、OUTPUT はあるが A-9 未実行（タイトル空）も「先に A-9」ダイアログで終了する（budget-nav-guard）。
  本ナビは予測を一切実行しない。
- **非保持（現行仕様 / 設計判断の保留点）**：A-9 を再実行すると `resetOutputSheet_` が OUTPUT を全消去するため、
  H/I の手入力は消え、H は再び月次P50初期値・I は空欄に戻る（=一回入力前提）。EVAL_INSIGHTS の入力保持 upsert とは
  方針が異なる。**H/I を A-9 再実行をまたいで保持するか（upsert/別シート化）は未決の設計判断**（§12）。
...
現存（2.3.46）の latent issue：
- **CONFIG数値読取の空セルの罠**（§3.3）：`getCfg`（readModelTuningFromConfig_ 内）と `readAiMissingConfidenceDefault_` が
  空セル('')を `Number('')===0` 経由で 0 として採用し、既定値でなくクランプ下限へ化ける。正常シード済み book では非発火。
  是正プロンプト `config-blank-guard` は用意済み・未適用。
- **OUTPUT の coerceMatch / coerceCount 死にコード**：`writeOutputFY_` 内の `warn_coerced=` 判定は、Vertex 警告サマリーに
  当該文字列が含まれないため恒常的に非発火（coerceCount=0 で if(>=3) は永久に走らない）。無害だが死にコード。
  `config-blank-guard` で同時に削除予定（未適用）。
- **予算策定欄 H/I の A-9 再実行非保持**（§9.6）：`resetOutputSheet_` が都度ワイプするため H/I 手入力が消える。
  「一回入力前提」か「再実行をまたいで保持（upsert/別シート）」かは**未決の設計判断**。EVAL_INSIGHTS の保持 upsert と方針が異なる。
...
- calibration override の空欄/明示0 取り違えは calibration-blank-override-fix で解消済み（※ getCfg 側は §3.3 のとおり未是正）。
...
この v2.3.9 は、annual-forecast-mode（FORECAST_CLOSED_MONTH_MODE）の方針「A=通年予測」、client-match-unify /
a9-client-normalize の正規化統一、overlay-cap-raise（主観月次cap 0.40・AI上限 ±5%）、spot-bg-sensitivity-down
（背景SPOT shrink 0.35・spike MAD_K 2.5）、report-text-compress（AI調査要約の短文圧縮）を維持しつつ、2.3.42〜2.3.46 の
5増分（dashboard-menu-to-b による DASHBOARD の B-3 移設、output-budget-columns / budget-entry-polish による OUTPUT 混合
セクションの予算策定欄 H/I/J 追加、guide-a10-sync、budget-nav-guard による A-10 ナビゲーションのガード）を現行実装へ同期した
ドキュメント改訂である。これら5増分はいずれも表示・メニュー・ナビゲーションのみの変更で、スコア算出式・予測コア・履歴スキーマは
無変更、A-9 の年度合計・月次 P10/P50/P90 は不変。年度合計は常に12ヶ月すべての予測simから算出する通年予測であり、実績は検証・
学習経路（B/C）でのみ使用する。予算策定欄は OUTPUT 上での手入力確定欄であり計画の主数値（混合 P50）そのものではない。
ブロッキングな残存 latent issue は無い（config-blank-guard で是正予定の空セルの罠・死にコードは非ブロッキング）。
